...
const STAGES = [
  ...
  { id: "gap", label: "Gap analysis e plays", sub: "Situação atual vs. sua entrada" },
      ...
      >
        <div className="max-w-3xl mx-auto px-5 py-3 flex items-center gap-3">
          <button
            ...
            aria-label="Meus planos salvos"
            className="flex items-center gap-1.5 px-2.5 py-1.5"
            style={{ background: t.segTrack, border: "none", borderRadius: 10, cursor: "pointer", color: t.ink }}
          >
            <span style={{ fontSize: 15, lineHeight: 1 }}>☰</span>
            {saved.length > 0 && (
              <span style={{ ...mono, fontSize: 11, fontWeight: 700, color: mode === "light" ? t.forest : t.bright }}>{saved.length}</span>
            )}
          </button>
          ...
          <article className="mt-8">
            ...
            <div className="no-print mt-8">
              <div className="flex gap-3 flex-wrap">
                ...
                >
                  {currentSaved ? "✓ Plano salvo" : "Salvar este plano"}
                </button>
              </div>
              <p className="mt-2 text-xs" style={{ color: t.gray }}>
        ...
        <div className="no-print" style={{ position: "fixed", inset: 0, zIndex: 100 }}>
          ...
          >
            <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${t.mist}` }}>
              <span style={{ ...display, fontWeight: 700, fontSize: 17 }}>Meus planos</span>
              <button onClick={() => setDrawerOpen(false)} aria-label="Fechar" style={{ background: t.segTrack, border: "none", borderRadius: 999, width: 30, height: 30, cursor: "pointer", color: t.ink, fontSize: 16 }}>×</button>
            </div>
            <div style={{ overflowY: "auto", padding: 16, flex: 1 }}>
              {saved.length === 0 ? (
                <div className="text-sm" style={{ color: t.gray, padding: "24px 8px", textAlign: "center" }}>
                  ...
                  <br />
                  Gere um plano e toque em "Salvar este plano".
                </div>
              ) : (
                  ...
                  {saved.map((s) => (
                    <div key={s.id} className="p-3" style={{ background: t.field, borderRadius: 12 }}>
                      <p className="text-sm font-semibold" style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {s.empresa || "Conta sem nome"}
                      </p>
                      <p className="text-xs mt-0.5" style={{ color: t.gray }}>
                        {s.vendedora} · {new Date(s.data).toLocaleDateString("pt-BR", { day: "2-digit", month: "short", year: "numeric" })}
                      </p>
                      <div className="flex gap-2 mt-2">
                        <button onClick={() => openSaved(s)} className="px-3 py-1.5 text-xs font-semibold" style={{ background: t.soft, color: mode === "light" ? t.forest : t.bright, border: "none", borderRadius: 999, cursor: "pointer" }}>
                          Abrir
                        </button>
                        <button onClick={() => deleteSaved(s.id)} className="px-3 py-1.5 text-xs font-semibold" style={{ background: "transparent", color: t.gray, border: "none", borderRadius: 999, cursor: "pointer" }}>
                          Excluir
                        </button>
                      </div>
                    </div>
            ...
            <p className="px-5 py-3 text-xs" style={{ color: t.gray, borderTop: `1px solid ${t.mist}` }}>
              Salvos apenas neste navegador. Máximo de 30 planos.
            </p>
          </aside>
        </div>
      )}
...
function RevenueBars({ t, data }) {
  ...
  return (
    <div className="mt-2" style={{ overflowX: "auto" }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", minWidth: 320, height: "auto", display: "block", overflow: "visible" }} role="img" aria-label="Evolução do faturamento anual">
  ...
  return (
    <div className="mt-5 pt-4" style={{ borderTop: `1px solid ${t.mist}` }}>
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <p style={{ ...mono, fontSize: 11, color: t.gray, letterSpacing: "0.06em" }}>COMPARATIVO TRIMESTRAL ({moeda || ""}) · FONTE: ITR/RI</p>
      ...
      {prevAno && (
        <p className="mt-1 text-xs" style={{ color: t.gray }}>
          Barras claras: {prevAno} · Barras em destaque: {sel} · Variação exibida é ano contra ano por trimestre.
        </p>
      )}
    </div>
  );
