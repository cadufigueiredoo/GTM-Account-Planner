...
const THEMES = {
  light: {
    ...
    tableHead: "#F6F7F9",
    headerBg: "rgba(237,238,242,0.72)",
    headerText: "#0A0A0A",
    zebra: "#FAFBFC",
    danger: "#FF3B30",
    field: "#F1F2F5",
    shadow: "0 1px 1px rgba(16,24,40,0.04), 0 4px 12px rgba(16,24,40,0.06), 0 16px 40px rgba(16,24,40,0.06)",
    shadowSoft: "0 1px 2px rgba(16,24,40,0.04), 0 6px 16px rgba(16,24,40,0.05)",
    segTrack: "rgba(118,118,128,0.10)",
    segActive: "#FFFFFF",
    atmo: "radial-gradient(900px 500px at 15% -5%, rgba(159,232,112,0.10), transparent 60%), radial-gradient(1000px 600px at 100% 0%, rgba(22,51,0,0.06), transparent 55%)",
  },
  dark: {
    paper: "#0A0B0D",
    card: "#17181B",
    ...
    forest: "#163300",
    bright: "#9FE870",
    soft: "rgba(159,232,112,0.14)",
    tableHead: "#202226",
    headerBg: "rgba(10,11,13,0.68)",
    headerText: "#F5F5F7",
    zebra: "#1C1E21",
    danger: "#FF453A",
    field: "#212226",
    shadow: "0 1px 1px rgba(0,0,0,0.4), 0 8px 24px rgba(0,0,0,0.45), 0 24px 60px rgba(0,0,0,0.4)",
    shadowSoft: "0 1px 2px rgba(0,0,0,0.4), 0 6px 18px rgba(0,0,0,0.4)",
    segTrack: "rgba(120,120,130,0.20)",
    segActive: "#3A3B40",
    atmo: "radial-gradient(900px 500px at 15% -5%, rgba(159,232,112,0.08), transparent 60%), radial-gradient(1000px 600px at 100% 0%, rgba(159,232,112,0.05), transparent 55%)",
  },
};
...
// ---------- Etapas visuais ----------
const STAGES = [
  { id: "research", label: "Account research", sub: "Web: size, market, news and recent signals" },
  { id: "contexto", label: "Context & stakeholders", sub: "Overview, initiatives, power map" },
  { id: "gap", label: "Gap analysis & plays", sub: "Account current stack vs. your offer" },
  { id: "acao", label: "Action plan & risks", sub: "90 days, mitigation" },
  { id: "pitch", label: "Core pitch", sub: "4C framework, 15 minutes" },
  { id: "disc", label: "Discovery & objections", sub: "MEDDIC and rebuttals" },
  { id: "analise", label: "Strategic analysis", sub: "SWOT, revenue and recommendations" },
];
...
// English version of the sample plan — lets the PT/EN toggle switch the demo
// instantly, with no API call (mirrors the Integrated Campaign Intelligence tool).
// Numbers, real names, tier and confiabilidade tokens are kept identical to DEMO_PLAN.
const DEMO_PLAN_EN = {
  tier: "HIGH",
  potencial: "US$ 8-15M/year in FX volume",
  visaoGeral:
    "Infracommerce is Latin America's largest full-commerce platform, running the entire e-commerce chain (platform, logistics, payments, and customer service) for major brands. It operates in Brazil, Mexico, Chile, Colombia, Argentina, and Peru, generating a constant flow of receivables and payments across multiple currencies. The multi-country operation creates FX and treasury complexity that grows with every new geography.",
  iniciativas: [
    "Consolidation of LATAM operations after a cycle of regional acquisitions",
    "Pressure for margin efficiency and operational cost reduction",
    "Expansion of embedded financial services for client brands",
    "Standardization of treasury and reconciliation systems across countries",
  ],
  insight:
    "The ideal entry point is treasury: batch payments to suppliers and transfers between LATAM entities currently go through banks with opaque spreads and SWIFT lead times. A proof of value on a single corridor (Brazil-Mexico) demonstrates measurable savings within weeks.",
  stakeholders: [
    { nome: "CFO (to confirm)", area: "Finance", papel: "Economic Buyer", sentimento: "Unknown", proximoPasso: "Executive briefing with an FX-savings business case" },
    { nome: "Head of Treasury", area: "Treasury", papel: "Technical Buyer", sentimento: "Unknown", proximoPasso: "Discovery on corridors and volumes by currency" },
    { nome: "Controller LATAM", area: "Controllership", papel: "User Buyer", sentimento: "Unknown", proximoPasso: "Map the multi-country reconciliation pain" },
    { nome: "Head of Financial Ops", area: "Operations", papel: "Potential Champion", sentimento: "Unknown", proximoPasso: "Initial BDR outreach via LinkedIn" },
  ],
  gaps: [
    { categoria: "FX and remittances", atual: "Incumbent banks + per-country brokers", maturidade: "2/5", gapWise: "Mid-market rate with transparent fees; typical 1-3% savings per transaction, measurable in the P&L" },
    { categoria: "Accounts abroad", atual: "Local bank accounts in each country, high fixed cost", maturidade: "3/5", gapWise: "Multi-currency account with local bank details in USD/EUR/GBP and balances in 40+ currencies" },
    { categoria: "Supplier payouts", atual: "Manual SWIFT, 2-5 days, spreadsheet reconciliation", maturidade: "2/5", gapWise: "Batch payments with tracking and reconciliation via API; treasury hours recovered" },
    { categoria: "Infra / API", atual: "No FX-ERP integration; manual processes", maturidade: "1/5", gapWise: "Wise Platform to embed international payments into the finance flow and the product" },
  ],
  plays: [
    { titulo: "Batch payments for international suppliers", produtos: "Wise Business + Batch", potencial: "Estimated savings of US$ 150-400K/year", descricao: "Migrate recurring payments to overseas suppliers and partners to batches at the mid-market rate. Proof of value on one corridor in 30 days.", esforco: "Low", impacto: "High", trigger: "How much spread does your treasury pay today on Brazil-Mexico payments? Most don't know, because the cost is embedded in the rate." },
    { titulo: "LATAM receivables consolidation", produtos: "Multi-currency account", potencial: "Reduced multi-country bank fixed costs", descricao: "Centralize MXN, CLP, COP, and ARS receivables in a multi-currency account, converting at the optimal moment instead of forced per-country conversions.", esforco: "Medium", impacto: "High", trigger: "Did regional acquisitions leave a different bank in each country? That's the pattern that generates the most invisible treasury cost." },
    { titulo: "Embedded financial services for brands", produtos: "Wise Platform (API)", potencial: "New B2B2C revenue line", descricao: "Embed international payments into the full-commerce offering, letting client brands sell and settle cross-border without building their own infrastructure.", esforco: "High", impacto: "High", trigger: "Do your client brands ask for international expansion? Settlement infrastructure can be a differentiator of your platform, not a cost to them." },
  ],
  acoes: [
    { quando: "W1-W2", acao: "Map treasury and financial-ops stakeholders on LinkedIn", objetivo: "Identify a potential champion" },
    { quando: "W2", acao: "Personalized outbound with a LATAM FX-cost insight", objetivo: "Generate a first reply" },
    { quando: "W3-W4", acao: "Cold call + follow-up with a multi-country e-commerce case", objetivo: "Qualify corridors and volumes" },
    { quando: "M2", acao: "30-min discovery with treasury; executive briefing with the CFO", objetivo: "Map real volumes by currency" },
    { quando: "M3", acao: "Proposal with proof of value: one corridor, measured savings", objetivo: "Advance to a pilot" },
  ],
  riscos: [
    { risco: "Entrenched banking relationship (credit tied in)", prob: "High", impacto: "High", mitigacao: "Position as a complement for payments, not a replacement for the credit bank" },
    { risco: "Conservative compliance/treasury", prob: "Medium", impacto: "Medium", mitigacao: "Wise's licenses and audit trail; low-volume pilot" },
    { risco: "Competitor already in the account (Payoneer/Ebury)", prob: "Medium", impacto: "Medium", mitigacao: "Real total-cost comparison on a specific corridor" },
    { risco: "Long decision cycle (enterprise)", prob: "High", impacto: "Medium", mitigacao: "Low-friction entry point with measurable savings in 30 days" },
  ],
  pitch: {
    abertura: "E-commerce companies operating in 4+ Latin American countries spend on average 1.5% to 3% of their international volume on embedded FX spread — a cost that rarely shows up in reports because it sits inside the rate.",
    problema: "Infracommerce's multi-country operation inherits a different bank and process per geography. The result is opaque spread, SWIFT lead times, and manual reconciliation that grows with every acquisition.",
    solucao: "Wise Business centralizes receivables in a multi-currency account, runs batch payments at the mid-market rate with transparent fees, and Wise Platform even lets you embed this into the offering to client brands. A pilot corridor demonstrates the savings in 30 days.",
    cta: "Would it make sense for me to show in 15 minutes how much a single corridor — Brazil-Mexico, for example — costs today versus on Wise?",
  },
  descoberta: [
    "[Metrics] Which corridors concentrate your international volume today (e.g., BRL-MXN, BRL-USD), what is the monthly volume of each, and how do you measure the effective FX cost versus the mid-market rate?",
    "[Economic Buyer] Besides you, who else would be involved in a decision about international payments infrastructure? How does the CFO view this topic today?",
    "[Decision] If you evaluated an international payments provider, what would the 3 most important criteria be, and how does the approval process for a project of this scale work?",
    "[Pain] In practice, how long does an international payment take from order to beneficiary confirmation? And how many people and spreadsheets touch that process after the regional acquisitions?",
    "[Champion] Is there someone in treasury or financial ops who has already mapped this cost and is actively looking for an alternative?",
  ],
  objecoes: [
    { objecao: "We already have a strong banking relationship and tied credit lines.", resposta: "Perfect, and it doesn't need to change. Wise comes in as a payments layer, not a credit layer. You keep the bank for funding and migrate only the transactional flow where the spread hurts. Many clients start with a single corridor." },
    { objecao: "A fintech for enterprise volumes? I need security and compliance.", resposta: "Fair. Wise is publicly listed (LSE), regulated in every market where it operates, with local licenses and a full audit trail per transaction. I can bring our compliance team for a technical session with yours." },
    { objecao: "We already use Payoneer for part of the payouts.", resposta: "Great sign: you've already validated that a bank isn't the only route. The conversation then becomes an objective comparison: total cost on a real corridor, the conversion rate applied, and currency coverage. Up for running that comparison with a sample of 10 payments?" },
  ],
  fitScore: 82,
  fitRazoes: [
    "Multi-country treasury pain maps directly to the core offering",
    "Estimated international volume of US$ 8-15M/year supports a HIGH tier",
    "Recent restructuring creates a supplier-review window",
  ],
  sinais: [
    { sinal: "Post-acquisition consolidation across 6 countries", tipo: "Restructuring", implicacao: "A natural review of inherited banks and processes opens room to consolidate payments." },
    { sinal: "Public pressure for margin efficiency", tipo: "Financial", implicacao: "Invisible FX cost becomes an executive topic when margins tighten." },
    { sinal: "Expansion of financial services for brands", tipo: "Expansion", implicacao: "Wise Platform fits as infrastructure for the new revenue line." },
  ],
  matriz: [
    { persona: "CFO (to confirm)", dor: "Pressured margin and invisible FX cost in the P&L", mensagem: "A pilot corridor quantifies in 30 days how much the spread is costing per year.", canal: "Executive meeting" },
    { persona: "Head of Treasury", dor: "Manual reconciliation and SWIFT lead times across 6 countries", mensagem: "Batch payments with an audit trail eliminate the reconciliation spreadsheet.", canal: "LinkedIn InMail" },
    { persona: "Controller LATAM", dor: "Slow, error-prone multi-currency close", mensagem: "Unified statements by currency cut the close by days.", canal: "Email" },
    { persona: "Head of Financial Ops", dor: "Inherited payout processes that differ by country", mensagem: "A single payout flow for all 6 countries, with an API into the ERP.", canal: "Cold call" },
  ],
  swot: {
    forcas: ["Full-commerce leadership in Latin America", "Consolidated operation across 6 countries", "Deep relationships with major brands"],
    fraquezas: ["Fragmented treasury inherited from acquisitions", "Manual multi-country reconciliation", "Invisible FX cost embedded in bank rates"],
    oportunidades: ["Brazil-Mexico pilot corridor with measurable savings", "LATAM receivables consolidation in a multi-currency account", "Embedded finance as new revenue for client brands"],
    ameacas: ["Banking relationship tied to credit", "Payoneer already present in part of the payouts", "Long enterprise decision cycle"],
  },
  faturamento: [
    { ano: "2023", valor: 3.1 },
    { ano: "2024", valor: 3.4 },
  ...
  moeda: "R$ bi (illustrative example)",
  confiabilidade: "estimado",
  sintese:
    "Infracommerce combines the three priority-account criteria: relevant international volume, structural treasury pain inherited from acquisitions, and a moment of margin pressure that makes invisible FX cost an executive topic. The timing favors entering now, before the renewal of regional banking agreements.",
  recomendacoes: [
    { titulo: "Open with the Brazil-Mexico corridor", detalhe: "Lowest-friction entry point with measurable savings in 30 days; it's the proof of value that unlocks the rest." },
    { titulo: "Multi-thread from week 1", detalhe: "Treasury (Technical Buyer) and Financial Ops (Champion) in parallel; the CFO enters via a business case, not a cold call." },
    { titulo: "Neutralize Payoneer with a comparison", detalhe: "Turn the competitor's presence into an advantage: propose an objective total-cost comparison on 10 real payments." },
  ],
  proximoPasso: "Map the 4 stakeholders on LinkedIn and send the opening InMail with the Brazil-Mexico corridor insight this week.",
};

// Cadência de prospecção Tier 1 (metodologia GTM enterprise) — estática
const CADENCIA_T1 = [
...
export default function App() {
  ...
  const [form, setForm] = useState({ empresa: "", segmento: SEGMENTOS[0], porte: "Mid-market (50 to 500 employees)", contexto: "" });
  const [status, setStatus] = useState({});
  const [plan, setPlan] = useState(null);
  const [running, setRunning] = useState(false);
  const [error, setError] = useState(null);
  const [debug, setDebug] = useState(null);
  // Output language of the CURRENT plan content. Generation is always PT; the
  ...
  const [saved, setSaved] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("gtm-planner-saved") || "[]");
    } catch (e) {
      return [];
    }
  });
  const [drawerOpen, setDrawerOpen] = useState(false);
  const currentSaved = plan && plan.visaoGeral && runRef.current.saveId ? saved.some((s) => s.id === runRef.current.saveId) : false;

  function persistSaved(list) {
    setSaved(list);
    try {
      localStorage.setItem("gtm-planner-saved", JSON.stringify(list));
    } catch (e) {}
  }

  function saveCurrentPlan() {
    if (!plan || !plan.visaoGeral) return;
    const id = runRef.current.saveId || (runRef.current.saveId = Date.now());
    const entry = { id, empresa: form.empresa, vendedora: seller.nome, data: new Date().toISOString(), form: { ...form }, seller: { ...seller }, plan, lang: contentLang };
    persistSaved([entry, ...saved.filter((s) => s.id !== id)].slice(0, 30));
  }

  const ALL_DONE = { research: "done", contexto: "done", gap: "done", acao: "done", pitch: "done", disc: "done", analise: "done" };

  function openSaved(entry) {
    setForm(entry.form);
    setSeller(entry.seller);
    setPlan(entry.plan);
    setStatus(ALL_DONE);
    setError(null);
    setDebug(null);
    setContentLang(entry.lang === "EN" ? "EN" : "PT");
    runRef.current = { brief: "saved-" + entry.id, intel: "saved", saveId: entry.id };
    setDrawerOpen(false);
  ...
  async function generate() {
    ...
    setError(null);
    setDebug(null);
    setContentLang("PT"); // generation is always in Portuguese; translate afterwards
    ...
    if (!sameRun) {
      setPlan({});
      setStatus({});
      runRef.current = { brief, intel: "", saveId: Date.now() };
    }
    ...
    if (!intel) {
      ...
      const researchPrompt = `IMPORTANTE: seja rápido; faça NO MÁXIMO 3 buscas na web e responda de forma enxuta. Pesquise a empresa "${form.empresa}" (Brasil, segmento ${form.segmento}). Resuma em até 10 bullets curtos: o que faz, porte estimado, presença internacional (países, moedas prováveis), expansão ou notícias recentes, fornecedores/clientes no exterior, sinais de dores que a oferta da vendedora resolve, NOMES REAIS de executivos relevantes (C-level, diretores das áreas-alvo) se encontrados publicamente, FATURAMENTO: priorize fontes primárias na busca (página de Relações com Investidores da empresa, ITRs, CVM, B3, balanços) e depois imprensa de negócios; traga o faturamento ANUAL dos últimos 3 anos ano a ano e, se a empresa for de capital aberto e os ITRs trouxerem, também o faturamento TRIMESTRAL por ano, e SINAIS DE COMPRA recentes: troca de executivos, captação de investimento, expansão, reestruturação, fusões, contratações em massa. Seja factual e conciso.`;
      try {
        intel = await withRetry(
          () => callClaude([{ role: "user", content: researchPrompt }], [{ type: "web_search_20260209", name: "web_search" }], 2000),
          2
        );
        setStatus((s) => ({ ...s, research: "done" }));
      } catch (e) {
        console.error("research", e);
        setDebug("Web research failed (" + String(e.message).slice(0, 80) + "); using general knowledge.");
        try {
          intel = await genText(
    ...
    if (failedUi.size) {
      const names = { contexto: "Context", gap: "Gap analysis", acao: "Action plan", pitch: "Pitch", disc: "Discovery", analise: "Strategic analysis" };
      setError(`Pending sections: ${[...failedUi].map((f) => names[f]).join(", ")}. Click "Generate Account Plan" to complete only what is missing.`);
    }
    setRunning(false);
  }
  ...
  function loadDemo() {
    ...
    setPlan(contentLang === "EN" ? DEMO_PLAN_EN : DEMO_PLAN); // matches the current toggle
    setStatus({ research: "done", contexto: "done", gap: "done", acao: "done", pitch: "done", disc: "done", analise: "done" });
    setError(null);
    setDebug(null);
  }

  // Switches the generated plan between Portuguese and English. The sample plan
  // ships in both languages, so it swaps instantly with no API call. Real plans
  // are translated field-by-field in small batches so the JSON never overruns
  // the output budget. Numbers, real names, and the tier/confiabilidade tokens
  // (which drive UI logic) are preserved; only prose values change.
  async function translatePlan(target) {
    if (translating || target === contentLang || !plan || !plan.visaoGeral) return;

    // Demo: instant, offline swap between the two hardcoded versions.
    if (runRef.current.brief === "demo") {
      setPlan(target === "EN" ? DEMO_PLAN_EN : DEMO_PLAN);
    ...
    setError(null);
    try {
      const langName = target === "EN" ? "English" : "Brazilian Portuguese";
      const sys =
        "You are a professional B2B sales translator. Return ONLY valid minified JSON, no prose, no markdown. " +
        "Translate every string VALUE into " + langName + ". Never translate or rename JSON keys. " +
        "Never alter numbers, and keep the exact same arrays and object shapes. " +
        "Keep unchanged: proper names of real people and companies, the stakeholder role taxonomy " +
        "(Economic Buyer, Technical Buyer, User Buyer, Champion, Gatekeeper), and these tokens: " +
        "MEDDIC, ICP, LGPD, LinkedIn, InMail, SWOT, LATAM, SWIFT, API, CFO, CEO, COO, CRO, CMO, ERP.";
      const p = plan;
      const groups = [
        { potencial: p.potencial, visaoGeral: p.visaoGeral, insight: p.insight, sintese: p.sintese, proximoPasso: p.proximoPasso, iniciativas: p.iniciativas, fitRazoes: p.fitRazoes, moeda: p.moeda },
        { stakeholders: p.stakeholders, matriz: p.matriz },
        { gaps: p.gaps, plays: p.plays },
        { acoes: p.acoes, riscos: p.riscos, sinais: p.sinais },
        { pitch: p.pitch, descoberta: p.descoberta, objecoes: p.objecoes },
        { swot: p.swot, recomendacoes: p.recomendacoes },
      ];
      let next = { ...p };
      const failed = [];
      ...
      setContentLang(target);
      if (failed.length) {
        setError("Translated, except: " + failed.join("; ") + ". Toggle the language again to retry those sections.");
      }
    } catch (e) {
      setError("Translation failed (" + String(e.message).slice(0, 90) + "). The plan was kept in the previous language.");
    } finally {
  ...
  return (
    <div style={{ ...body, backgroundColor: t.paper, backgroundImage: t.atmo, backgroundAttachment: "fixed", color: t.ink, minHeight: "100vh", colorScheme: mode, WebkitFontSmoothing: "antialiased" }}>
      ...
      >
        <div className="max-w-5xl mx-auto px-5 py-3" style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", gap: 12 }}>
          {/* Zona esquerda: My Plans */}
          ...
          {/* Zona central: marca */}
          <div style={{ justifySelf: "center", textAlign: "center", lineHeight: 1.15 }}>
            <div className="flex items-center justify-center gap-2">
              <span style={{ width: 9, height: 9, borderRadius: 999, background: t.bright, display: "inline-block", boxShadow: `0 0 0 4px ${t.soft}` }} />
          ...
          {/* Zona direita: contexto + tema */}
          <div style={{ justifySelf: "end" }} className="flex items-center gap-2">
            <span
                ...
                {plan.recomendacoes && (
                  <div className="mt-3 grid gap-3 sm:grid-cols-3">
                    {plan.recomendacoes.map((r, i) => (
                      <div key={i} className="rounded-xl p-4" style={{ borderRight: `1px solid ${t.mist}`, borderBottom: `1px solid ${t.mist}`, borderLeft: `1px solid ${t.mist}`, background: t.card, borderTop: `3px solid ${t.bright}` }}>
...
// ---------- Componentes ----------
// PT/EN toggle for the generated plan's output language (mirrors the theme
// segmented control). Disabled while a translation request is in flight.
function LangToggle({ t, lang, onChange, busy }) {
  const opts = ["PT", "EN"];
  return (
    <div
      role="tablist"
      aria-label="Plan language"
      className="flex p-0.5"
      style={{ background: t.segTrack, borderRadius: 10, opacity: busy ? 0.6 : 1 }}
      title={busy ? "Translating…" : "Plan language"}
    >
      {opts.map((o) => {
        const active = lang === o;
        return (
...
function Table({ t, head, rows }) {
  return (
    <div className="overflow-x-auto" style={{ border: `1px solid ${t.hair}`, borderRadius: 16, boxShadow: t.shadowSoft }}>
...
function SwotCard({ t, tone, label, items }) {
  ...
  return (
    <div className="p-4" style={{ borderTop: `1px solid ${t.hair}`, borderRight: `1px solid ${t.hair}`, borderBottom: `1px solid ${t.hair}`, background: t.card, borderRadius: 16, borderLeft: `4px solid ${accent}`, boxShadow: t.shadowSoft }}>
...
function RevenueBars({ t, data }) {
  ...
  return (
    <div className="mt-2" style={{ overflowX: "auto" }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", minWidth: 320, height: "auto", display: "block", overflow: "visible" }} role="img" aria-label="Annual revenue trend">
...
function QuarterChart({ t, mode, data, moeda }) {
  ...
  // point to a year the new plan doesn't have (chart would show only dashes
  // until the user clicks). Snap back to the latest available year when that
  // happens, instead of relying on the user to notice and fix it.
  useEffect(() => {
    if (!anos.includes(sel)) setSel(anos[anos.length - 1]);
  }, [data]); // eslint-disable-line react-hooks/exhaustive-deps
  const cur = data[sel] || [];
  const prevAno = anos[anos.indexOf(sel) - 1];
  const prev = prevAno ? data[prevAno] : null;
  const all = [...cur, ...(prev || [])].map((q) => Number(q.valor) || 0);
  const max = Math.max(...all, 0.0001);
  const acc = mode === "light" ? t.forest : t.bright;
  return (
    <div className="mt-5 pt-4" style={{ borderTop: `1px solid ${t.mist}` }}>
        ...
        <div className="flex p-0.5" style={{ background: t.segTrack, borderRadius: 10 }}>
          {anos.map((a) => (
            <button
              ...
              onClick={() => setSel(a)}
              className="px-3 py-1 text-xs font-semibold"
              style={{ background: sel === a ? t.segActive : "transparent", color: sel === a ? t.ink : t.gray, border: "none", borderRadius: 8, cursor: "pointer", boxShadow: sel === a ? "0 1px 4px rgba(0,0,0,0.12)" : "none" }}
            >
              {a}
            </button>
          ))}
      ...
      <div className="mt-4 flex items-end gap-4" style={{ height: 170 }}>
        {["T1", "T2", "T3", "T4"].map((tri) => {
          const c = cur.find((q) => q.tri === tri);
          const p = prev ? prev.find((q) => q.tri === tri) : null;
          const cv = c ? Number(c.valor) : null;
          const pv = p ? Number(p.valor) : null;
          const yoy = cv != null && pv ? ((cv - pv) / pv) * 100 : null;
          return (
            <div key={tri} className="flex flex-col items-center justify-end gap-1" style={{ flex: 1, height: "100%" }}>
              {cv != null && (
                <div className="text-center" style={{ lineHeight: 1.2 }}>
                  {yoy != null && (
                    <span className="block" style={{ fontSize: 10, fontWeight: 700, color: yoy >= 0 ? "#34A853" : t.danger }}>
                      {(yoy >= 0 ? "+" : "") + yoy.toFixed(1)}%
                    </span>
                  )}
              ...
              <div className="flex items-end gap-1" style={{ width: "100%", justifyContent: "center", height: 100 }}>
                {pv != null && <div title={`${prevAno}: ${pv}`} style={{ width: 16, height: Math.max(6, (pv / max) * 96), background: t.mist, borderRadius: "5px 5px 0 0" }} />}
                {cv != null ? (
                  <div title={`${sel}: ${cv}`} style={{ width: 22, height: Math.max(6, (cv / max) * 96), background: acc === t.forest ? "#163300" : "#9FE870", borderRadius: "6px 6px 0 0" }} />
                ) : (
                  <span className="text-xs" style={{ color: t.gray }}>—</span>
                )}
              </div>
              <span className="text-xs" style={{ color: t.gray }}>{tri}</span>
            </div>
          );
        })}
      </div>
...
function PitchCard({ t, label, c, accent }) {
  return (
    <div className="p-4" style={{ background: accent ? t.soft : t.card, border: `1px solid ${t.hair}`, borderRadius: 16, boxShadow: accent ? "none" : t.shadowSoft }}>
