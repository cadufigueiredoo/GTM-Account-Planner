...
export default function App() {
  ...
  function upsertPlan(finalPlan) {
    if (!finalPlan || !finalPlan.visaoGeral) return;
    const id = runRef.current.saveId || (runRef.current.saveId = Date.now());
    const entry = { id, empresa: form.empresa, vendedora: seller.nome, data: new Date().toISOString(), form: { ...form }, seller: { ...seller }, plan: finalPlan };
    persistSaved([entry, ...saved.filter((s) => s.id !== id)].slice(0, 20));
  }
  ...
  async function generate() {
    ...
    upsertPlan(currentPlan); // Meus planos: salva/atualiza automaticamente no navegador
    setRunning(false);
  }
  ...
  return (
    <div style={{ ...body, background: t.paper, color: t.ink, minHeight: "100vh", colorScheme: mode, WebkitFontSmoothing: "antialiased" }}>
      ...
      <main className="max-w-3xl mx-auto px-5 pb-24">
        ...
        {saved.length > 0 && (
          <section className="no-print mt-6 p-6" style={{ background: t.card, border: `1px solid ${t.mist}`, borderRadius: 20, boxShadow: t.shadow }}>
            <p style={{ ...mono, fontSize: 11, color: t.gray, letterSpacing: "0.08em" }}>MEUS PLANOS · SALVOS NESTE NAVEGADOR</p>
            <div className="mt-3 grid gap-2">
              {saved.map((s) => (
                <div key={s.id} className="flex items-center gap-3 px-3 py-2.5" style={{ background: t.field, borderRadius: 12 }}>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <p className="text-sm font-semibold" style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {s.empresa}
                      <span style={{ color: t.gray, fontWeight: 400 }}> · {s.vendedora}</span>
                    </p>
                  ...
                  <button onClick={() => deleteSaved(s.id)} aria-label="Excluir plano" className="px-2.5 py-1.5 text-xs font-semibold" style={{ background: "transparent", color: t.gray, border: "none", borderRadius: 999, cursor: "pointer" }}>
                    Excluir
                  </button>
                </div>
              ))}
