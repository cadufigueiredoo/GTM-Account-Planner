...
const CADENCIA_T1 = [
  { dia: "T+0", canal: "LinkedIn · Conexão", acao: "Solicitação de conexão sem mensagem. Deixe o perfil falar primeiro." },
  { dia: "T+2", canal: "LinkedIn · InMail", acao: "3 parágrafos: por que o contato, insight específico da dor que sua oferta resolve nesta conta, CTA de 15 min." },
  { dia: "T+5", canal: "E-mail · Conteúdo", acao: "Case de cliente do mesmo segmento. Não peça reunião: gere valor antes de vender." },
  { dia: "T+7", canal: "Cold call", acao: "Mencione o e-mail e o case. Valide se o tema ressoou e qualifique. 3-5 minutos." },
  { dia: "T+10", canal: "LinkedIn · Post", acao: "Comentário inteligente em publicação recente do stakeholder. Visibilidade sem abordagem." },
  { dia: "T+14", canal: "E-mail · Novo ângulo", acao: "Reabra com dado novo de mercado ou movimento de concorrente. CTA direto." },
  { dia: "T+21", canal: "Cold call · Break-up", acao: "'Quero respeitar seu tempo: faz sentido continuarmos?' Às vezes cria urgência." },
  { dia: "T+30", canal: "E-mail · Break-up", acao: "Feche o ciclo com elegância, porta aberta, conta em nurture." },
];
...
export default function App() {
  ...
  async function generate() {
    ...
    const SUB = [
      ...
      },
      {
        ...
        prompt: `${base}\n\nREGRA CRÍTICA: use SOMENTE números de faturamento que constem no INTEL DA PESQUISA acima. NÃO estime, NÃO invente. Se o intel não trouxer números anuais confiáveis, retorne {"faturamento":[],"moeda":"","confiabilidade":"indisponível"}. Caso existam, gere: {"faturamento":[{"ano":"2023","valor":numero}] com até 3 anos em ordem cronológica,"moeda":"ex: R$ bi ou R$ mi","confiabilidade":"público (RI/imprensa) | estimado"}`,
        apply: (d) => (currentPlan = { ...currentPlan, ...d }),
      },
      {
        id: "rec",
  ...
  const inputStyle = { border: `1px solid ${t.mist}`, background: t.paper, color: t.ink };

  return (
    <div style={{ ...body, background: t.paper, color: t.ink, minHeight: "100vh" }}>
      <style>{`
        @media print { .no-print { display: none !important; } body { background: #fff !important; } }
        input:focus, select:focus, textarea:focus, button:focus-visible { outline: 2px solid ${t.bright}; outline-offset: 2px; }
        @media (prefers-reduced-motion: reduce) { * { animation: none !important; transition: none !important; } }
        @keyframes pulse { 0%,100% { opacity: 1 } 50% { opacity: .35 } }
        select option { color: #0E120B; }
      `}</style>

      <header className="no-print" style={{ background: t.headerBg }}>
        <div className="max-w-3xl mx-auto px-5 py-4 flex items-center gap-3">
          <span style={{ width: 12, height: 12, borderRadius: 999, background: t.bright, display: "inline-block" }} />
          <div>
            <span style={{ ...display, color: "#fff", fontWeight: 700, fontSize: 18, letterSpacing: "0.02em" }}>GTM Planner</span>
            <a
              href="https://www.linkedin.com/in/carloseduardovf/"
              target="_blank"
              rel="noopener noreferrer"
              className="block"
              style={{ ...mono, color: t.bright, fontSize: 10, letterSpacing: "0.08em", textDecoration: "none" }}
            >
              DEVELOPED BY CARLOS EDUARDO ↗
            </a>
          </div>
          <span style={{ ...mono, color: t.bright, fontSize: 11, marginLeft: "auto" }}>LATAM · B2B</span>
          <button
            onClick={() => setMode(mode === "light" ? "dark" : "light")}
            aria-label="Alternar tema"
            className="rounded-full px-3 py-1 text-xs font-semibold"
            style={{ background: t.bright, color: t.forest, cursor: "pointer" }}
          >
      ...
      <main className="max-w-3xl mx-auto px-5 pb-24">
        <section className="no-print mt-8 rounded-2xl p-6" style={{ background: t.card, border: `1px solid ${t.mist}` }}>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <h2 style={{ ...display, fontSize: 18, fontWeight: 700 }}>1 · Sua empresa (vendedora)</h2>
            <button
              onClick={applyWisePreset}
              className="rounded-lg px-3 py-1.5 text-xs font-semibold"
              style={{ background: t.soft, color: mode === "light" ? t.forest : t.bright, border: `1px solid ${t.bright}`, cursor: "pointer" }}
            >
        ...
        <section className="no-print mt-6 rounded-2xl p-6" style={{ background: t.card, border: `1px solid ${t.mist}` }}>
          ...
          <div className="mt-5 grid gap-4">
            ...
            <button
              ...
              className="rounded-lg px-5 py-3 font-semibold text-base"
              style={{
                background: running || !form.empresa.trim() || !sellerReady ? t.mist : t.bright,
            ...
            <button
              onClick={loadDemo}
              disabled={running}
              className="rounded-lg px-5 py-3 font-semibold text-sm"
              style={{ background: t.card, color: mode === "light" ? t.forest : t.bright, border: `1px solid ${mode === "light" ? t.forest : t.bright}`, cursor: running ? "not-allowed" : "pointer" }}
            >
          ...
          <article className="mt-8">
            ...
            {plan.plays && (
              <Section t={t} n="5" title="Oportunidades de entrada">
                <div className="grid gap-4">
                  {plan.plays.map((p, i) => (
                    <div key={i} className="rounded-xl p-4" style={{ border: `1px solid ${t.mist}`, background: t.card }}>
            ...
            <Section t={t} n="10" title="Cadência de abordagem — Tier 1">
              <p className="text-sm mb-3" style={{ color: t.gray }}>
                8 touchpoints em 30 dias, 100% personalizados. Regra: quanto maior o tier, mais manual e menos automatizado.
              </p>
              <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${t.mist}` }}>
                {CADENCIA_T1.map((c, i) => (
                  <div key={i} className="flex gap-3 px-4 py-3" style={{ background: i % 2 ? t.zebra : t.card, borderTop: i ? `1px solid ${t.mist}` : "none" }}>
                    <span style={{ ...mono, fontSize: 12, color: mode === "light" ? t.forest : t.bright, fontWeight: 700, minWidth: 44 }}>{c.dia}</span>
                    <div>
            ...
            <div className="no-print mt-8 flex gap-3">
              <button onClick={() => window.print()} className="rounded-lg px-5 py-3 font-semibold" style={{ background: mode === "light" ? t.forest : t.bright, color: mode === "light" ? "#fff" : t.forest, cursor: "pointer" }}>
                Exportar / imprimir
              </button>
            </div>
  ...
  return (
    <section className="mt-8">
      <div className="flex items-baseline gap-3">
        <span style={{ ...display, color: t.bright, fontWeight: 700, fontSize: 14, background: t.forest, borderRadius: 999, padding: "2px 10px" }}>{n}</span>
        <h3 style={{ ...display, fontSize: 20, fontWeight: 700 }}>{title}</h3>
      </div>
...
function Table({ t, head, rows }) {
  return (
    <div className="overflow-x-auto rounded-xl" style={{ border: `1px solid ${t.mist}` }}>
...
function RevenueBars({ t, data }) {
  const max = Math.max(...data.map((d) => Number(d.valor) || 0), 0.0001);
  return (
    <div className="mt-3 flex items-end gap-6" style={{ height: 150 }}>
      {data.map((d, i) => {
        const h = Math.max(10, (Number(d.valor) / max) * 110);
        const last = i === data.length - 1;
        return (
          <div key={i} className="flex flex-col items-center justify-end gap-1" style={{ flex: 1, height: "100%" }}>
            <span className="text-sm font-semibold">{d.valor}</span>
            <div
              style={{
                width: "100%",
                maxWidth: 90,
                ...
                background: last ? t.bright : t.mist,
                border: `1px solid ${last ? t.bright : t.mist}`,
                transition: "height .4s ease",
              }}
            />
            <span className="text-xs" style={{ color: t.gray }}>{d.ano}</span>
          </div>
        );
      })}
    </div>
